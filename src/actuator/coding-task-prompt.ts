...
export function buildCodingTaskPrompt(packet: WorkerTaskPacket): string {
  ...
  const prompt = [
    ...
    `- You may run the exact trusted repository test command locally while developing (${packet.targetTestCommand ?? "host-provided test"}) so you can inspect failures and repair your implementation before exiting.`,
    "- You may rerun that exact local test as needed during this bounded task, but do not alter the test to make incorrect code pass.",
    "- The trusted host will independently rerun the test after you exit; your local test result does not determine acceptance.",
