...
import { ApiError, handleApiError, jsonResponse } from "./errors.js";
...
async function handlePostEvidence(
  ...
  if (blobBase64 !== undefined) {
    const key = evidenceBlobKey(projectId, id);
    const bytes = decodeBase64ToBytes(blobBase64);
    const ref = await deps.blobs.put(key, bytes, mimeType);
    contentRef = ref.key;
  }
  ...
  const evidence = await deps.store.createEvidence({
    id,
    projectId,
    kind: validators.evidenceKind(body.kind, "kind"),
    label: assertNonEmptyString(body.label, "label"),
    ...(taskId !== undefined ? { taskId } : {}),
    ...(workerRunId !== undefined ? { workerRunId } : {}),
    ...(verifierRunId !== undefined ? { verifierRunId } : {}),
...
const routes: readonly {
  ...
  { method: "GET", pattern: "/health", handler: () => handleGetHealth() },
  {
    method: "GET",
  ...
  { method: "POST", pattern: "/api/projects", handler: (request, _params, deps) => handlePostProject(request, deps) },
  {
    method: "POST",
    pattern: "/api/requirements",
    handler: (request, _params, deps) => handlePostRequirement(request, deps),
  },
  { method: "POST", pattern: "/api/tasks", handler: (request, _params, deps) => handlePostTask(request, deps) },
  {
    method: "POST",
  ...
  { method: "POST", pattern: "/api/evidence", handler: (request, _params, deps) => handlePostEvidence(request, deps) },
  {
    method: "POST",
...
async function dispatch(request: Request, deps: ApiDependencies): Promise<Response> {
  const url = new URL(request.url);
  ...
  for (const route of routes) {
    ...
    if (params !== null) {
      return route.handler(request, params, deps);
    }
  }
...
export function createApiRouter(deps: ApiDependencies): (request: Request) => Promise<Response> {
